"""
Rebuild the live RC Mode C baskets (rcv2c_rc / rcv2c_full) with the TUNED anchor (30%,
down from 40%) — the sweep winner within the specified range. Default bounds (small tier
stays empty on RC, excluding the junkiest micro-caps). Overwrites just those two variants.

    python scripts/build_rcv2c_tuned.py
"""
...
ANCHOR = 0.30
VARIANTS = [("rcv2c_rc", "RupeeCase window", "2022-11-18"),
            ("rcv2c_full", "Full window", "2017-08-01")]


def build():
    b = EquinextV2RC()
    b.TIER_MODE = "C"; b.LARGE_ANCHOR = ANCHOR; b.SATELLITE_TILT = 1.0
    b.SMALL_CAP_MAX = 0.40; b.TIER_BOUNDS = (100, 250, 500)
    b.USE_TREND_GATE = False        # drop the 200-DMA: it cost ~4pp of drawdown for no return here
    return b
...
def main():
    ...
    for key, window, start_s in VARIANTS:
        ...
        print(f"computing {key} (anchor {int(ANCHOR*100)}%) ...", flush=True)
        cfg = BacktestConfig(start=start.date(), end=end.date(), rebalance="ME")
        res = run_backtest(build(), ctx, cfg)
        ...
        data["variants"][key] = {
            "label": "RC Stocks · Mode C", "window": window,
            "start": str(idx[0].date()), "end": str(idx[-1].date()),
            "strategy_v2": True, "mode_c": True, "momentum_only": True, "no_dma": True,
            "sector_capped": True, "cap_pct": 25, "anchor_pct": int(ANCHOR * 100),
            "metrics": m,
    ...
    print(f"\nrebuilt tuned rcv2c into {dest}  ({dest.stat().st_size // 1024} KB)")
